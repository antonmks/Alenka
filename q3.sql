O := LOAD 'orders' BINARY AS (o_orderkey{1}:int, o_custkey{2}:int, orderdate{5}:int, shippriority{8}:int);
OFI := FILTER O BY orderdate < 19950315;
C := LOAD 'customer' BINARY AS (c_custkey{1}:int, mktsegment{7}:varchar(10));
CF := FILTER C BY mktsegment == "BUILDING";
L := STREAM 'lineitem' BINARY AS (l_orderkey{1}:int,  price{6}:decimal, discount{7}:decimal, shipdate{11}:int);
