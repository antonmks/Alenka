...
#include "cm.cu"
    ...
    void emit_join_tab(char *s, int left);
    void emit_distinct();
...
void emit_select(char *s, char *f, int ll)
{
	...
	unordered_map<long long int, unsigned int> mymap; //this is where we keep the hashes of the records
    ...
    for(unsigned int i = 0; i < cycle_count; i++) {          // MAIN CYCLE
		...
		std::cout<< "select cycle time " <<  ( ( std::clock() - start2 ) / (double)CLOCKS_PER_SEC ) <<'\n';
    };
