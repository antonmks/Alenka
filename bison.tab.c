...
void emit_join(char *s, char *j1)
{
	...
	if(!left->prm.empty())
	    for(int i = 0; i < left->segCount; i ++) {  
	        rcount = rcount + (left->prm[setMap[f1]][i].end() - left->prm[setMap[f1]][i].begin());
		}	
	else
	    rcount = varNames[setMap[f1]]->mRecCount;
	cout << "allocing " << rcount << endl;	
	...
	if(!right->prm.empty())
	    for(int i = 0; i < right->segCount; i ++) {  
	        rcount = rcount + (right->prm[setMap[f2]][i].end() - right->prm[setMap[f2]][i].begin());
		}	
	else
	...
	while(!op_sel.empty()) {
	    if(c->prm.count(setMap[op_sel.front()]) == 0) {
	        ...
	        c->prm[setMap[op_sel.front()]].push_back(thrust::device_vector<unsigned int>(d_res1.size()));
		    if(lr->prm.size() != 0) {
		     // join prm segments, add seg_num*maxRecs to values and gather the result
				...
				for(unsigned int i = 0; i < lr->prm[setMap[op_sel.front()]].size(); i++) {												
			        ...
			        thrust::transform(lr->prm[setMap[op_sel.front()]][i].begin(), lr->prm[setMap[op_sel.front()]][i].end(),
				                      res.begin() + curr_count, _1+(i*t->maxRecs));									  
					curr_count = curr_count + lr->prm[setMap[op_sel.front()]][i].size();

                }
				if(left_check)
				    thrust::gather(d_res1.begin(), d_res1.end(), res.begin(), c->prm[setMap[op_sel.front()]][0].begin());						
				else	
				    thrust::gather(d_res2.begin(), d_res2.end(), res.begin(), c->prm[setMap[op_sel.front()]][0].begin());				
			}
            else { // copy d_res2 into prm[setMap[op_sel.front()]]
			 	if(left_check)
                    thrust::copy(d_res1.begin(), d_res1.end(), c->prm[setMap[op_sel.front()]][0].begin());
				else	
				    thrust::copy(d_res2.begin(), d_res2.end(), c->prm[setMap[op_sel.front()]][0].begin());
            }  			
		};	
		op_sel.pop();
	};
	...
	for  (map<string, std::vector<thrust::device_vector<unsigned int> > >::iterator it=left->prm.begin() ; it != left->prm.end(); ++it ) {        
        varNames[(*it).first]->deAllocOnDevice();
	};		
	for  (map<string, std::vector<thrust::device_vector<unsigned int> > >::iterator it=right->prm.begin() ; it != right->prm.end(); ++it ) {        
        varNames[(*it).first]->deAllocOnDevice();
	};		
...
void emit_order(char *s, char *f, int e, int ll)
{
    ...
    if(a->prm.size() != 0) {	
		for ( map<string, std::vector<thrust::device_vector<unsigned int> > >::iterator it=a->prm.begin() ; it != a->prm.end(); ++it ) {				
            b->prm[(*it).first].push_back(thrust::device_vector<unsigned int>(a->mRecCount));
			cout << (*it).first << endl;
		    thrust::gather(permutation, permutation+a->mRecCount, (*it).second[0].begin(), b->prm[(*it).first][0].begin());		
        };			
	}				
	else { 	    
        b->prm[a->name].push_back(thrust::device_vector<unsigned int>(a->mRecCount));
        thrust::copy(permutation, permutation+a->mRecCount, b->prm[a->name][0].begin());
    };
...
void emit_select(char *s, char *f, int ll)
{
	...
	//cout << "select mem " << getFreeMem() << endl;
	std::clock_t start2 = std::clock();	

	    if(i == 0)
            b = new CudaSet(0, col_count);		
		copyColumns(a, op_vx, i);	
		copyGather(a, op_vx, i, 1);	
		
		CudaSet *t = varNames[setMap[op_vx.front()]];
				
        if (ll != 0) {
            thrust::device_ptr<unsigned int> perm = order_inplace(a,op_v2,field_names,i);			
			//std::cout<< "cycle time3 " <<  ( ( std::clock() - start2 ) / (double)CLOCKS_PER_SEC ) <<'\n';
			
			//for(int z=0; z < 10;z++)
			//   cout << "DATA " << a->d_columns_float[0][z] << " " << a->d_columns_float[1][z] << " " << a->d_columns_float[2][z] << " " << a->h_columns_cuda_char[0]->d_columns[0][z] << " " << a->h_columns_cuda_char[1]->d_columns[0][z] << endl;
...
// save the filter parameters for possible later zone map filtering
    ...
    if(a->mRecCount == 0) {
	    ...
	    for(unsigned int i = 0; i < cycle_count; i++) {	
			...
			copyGather(a, op_value, i, 1);
		    filter(op_type,op_value,op_nums, op_nums_f,a, b, i);
		};	
		varNames[setMap[op_value.front()]]->mRecCount = varNames[setMap[op_value.front()]]->oldRecCount;			
